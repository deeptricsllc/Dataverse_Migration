...
export class InsightsService {
  ...
  async profile(ctx: RequestContext, environmentId: string, table: string): Promise<ProfileDto> {
    ...
    try {
      ...
      const columns = meta.attributes
        .filter((a) => !a.attributeOf && a.isValidForRead && !['Virtual', 'EntityName', 'Image', 'File', 'Other'].includes(a.type))
        .slice(0, 40);
      const sample: DvRecord[] = [];
      for await (const page of conn.queryRecords(meta, columns.map((c) => c.logicalName), { pageSize: PROFILE_SAMPLE })) {
        sample.push(...page);
      ...
      const nullStats = columns
        ...
        .map((c) => {
          const nulls = sample.filter((r) => r.values[c.logicalName] === null || r.values[c.logicalName] === undefined || r.values[c.logicalName] === '').length;
          return {
            field: c.logicalName,
      ...
      return {
        ...
        sampleRecords: sample.slice(0, 5).map((r) => ({
          id: r.id,
          values: Object.fromEntries(Object.entries(r.values).map(([k, v]) => [k, displayValue(byName.get(k), v)])),
        })),
      };
    } catch (err) {
  ...
  async dashboard(ctx: RequestContext): Promise<DashboardDto> {
    ...
    const valOutcome = await this.db
      ...
      .from(validationRuns)
      .where(and(eq(validationRuns.organizationId, ctx.organizationId), inArray(validationRuns.status, ['COMPLETED'])))
      .groupBy(validationRuns.outcome);
    const [valTotal] = await this.db.select({ n: count() }).from(validationRuns).where(eq(validationRuns.organizationId, ctx.organizationId));
    const byStatus = (s: string[]) => runStatus.filter((r) => s.includes(r.status)).reduce((n, r) => n + Number(r.n), 0);
    ...
    return {
      environments: { total: envs.length, connected: envs.filter((e) => e.connectionStatus === 'CONNECTED').length },
      migrationRuns: {
      ...
      validationRuns: { total: Number(valTotal?.n ?? 0), pass: byOutcome('PASS'), warning: byOutcome('WARNING'), fail: byOutcome('FAIL') },
