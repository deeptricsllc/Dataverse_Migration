 ...
 */
/**
 * Serializes inserts per simulated table. A real IDENTITY column allocates atomically even under
 * concurrent inserts; allocating and inserting must therefore happen as one step here, not as a
 * read followed by a write that another writer can interleave with.
 */
const tableLocks = new Map<string, Promise<unknown>>();

function withTableLock<T>(key: string, fn: () => Promise<T>): Promise<T> {
...
export class DemoSqlConnection implements MigrationConnector {
  ...
  /** The next IDENTITY value: one past the highest key currently in the table. */
  private async nextIdentity(logicalName: string): Promise<number> {
    const rows = await this.db
    ...
    }, 1000); // demo identities start above the seeded ids so they never collide
    return highest + 1;
  }
  ...
  private async insert(t: TableMetadata, record: WriteRecord, _options: WriteOptions): Promise<string> {
    ...
    if (pk.sql?.isIdentity) {
      id = String(await this.nextIdentity(table.logicalName));
