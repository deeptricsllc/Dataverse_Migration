...
export function RunsPage() {
  ...
  const runs = useQuery({
    queryKey: ['runs'],
    queryFn: () => get<MigrationRunListItemDto[]>('/api/runs'),
  ...
  return (
    <>
      <PageHeader
        title="Runs"
        description="Complete, persisted history of migration and validation runs in your organization."
      />
      <div className="mb-4">
        <Tabs
          value={tab}
          ...
          tabs={[
            ...
            {
              value: 'validation',
              label: `Validation runs${validations.data ? ` (${validations.data.length})` : ''}`,
            },
          ]}
        />
      </div>
      {tab === 'migration' && (
        <Card bodyClassName="p-0">
          ...
          {runs.data?.length === 0 && (
            <EmptyState icon={<History className="h-8 w-8" />} title="No migration runs yet" />
          )}
              ...
              <tbody className="divide-y divide-slate-100">
                {runs.data.map((r) => (
                  ...
                  >
                    ...
                    <Td className={`text-right tabular-nums ${r.failed ? 'text-red-700' : ''}`}>
                      {fmtNumber(r.failed)}
                    </Td>
    ...
    <Table>
      ...
      <tbody className="divide-y divide-slate-100">
        {items.map((v) => (
          ...
          >
            <Td>
              {v.sourceEnvironment.displayName} <ArrowRight className="inline h-3 w-3" />{' '}
              {v.targetEnvironment.displayName}
            </Td>
            <Td>
            ...
            <Td className="text-xs text-slate-500">
              {v.migrationRunId ? 'Migration run' : 'Environment tables'}
            </Td>
